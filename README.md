...
- **Zona horaria de la oficina:** las horas se manejan como la **hora-pared de la oficina** (`OFFICE_TZ`, por defecto `America/Mexico_City`). El servidor calcula el "ahora" en esa zona, de modo que al reservar para hoy a una hora aún futura el sistema no la marca erróneamente como pasada. El frontend también evalúa el "ahora" como hora-pared del navegador para mostrar correctamente los controles de **editar/cancelar** y de **asistencia** de las reservas de hoy.
