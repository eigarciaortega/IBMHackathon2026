...
import bookingApp from './index.js';
import { mountApiDocs } from '../../shared/openapiMount.js';

/** Endpoints HTTP del booking-service que la spec DEBE documentar (R13.2). */
const EXPECTED = [
  { path: '/disponibilidad', method: 'get', codes: [200, 400, 401, 500], request: 'params' },
...
describe('booking-service OpenAPI', () => {
  it('es un documento OpenAPI 3.0 válido', () => {
    const spec = buildSpec();
    expect(spec.openapi).toMatch(/^3\./);
    expect(spec.info?.title).toContain('booking-service');
  });
