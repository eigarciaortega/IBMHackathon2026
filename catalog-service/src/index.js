/**
 * IBM OfficeSpace · Catalog Service
 */
...
process.on('unhandledRejection', (err) => {
  console.error('[catalog-service] Promesa no manejada', err)
})

const server = app.listen(PORT, () => {
  console.log(`🏢 catalog-service en http://localhost:${PORT}`)
...
function shutdown(signal) {
  console.log(`[catalog-service] ${signal} recibido, cerrando…`)
  server.close(() => {
    pool
      .end()
      .then(() => process.exit(0))
